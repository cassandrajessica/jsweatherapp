...
const locationBtn = document.querySelector(".location-container");
...
// location button to get current location weather
locationBtn.addEventListener("click", () => {
  if (navigator.geolocation) {
    navigator.geolocation.getCurrentPosition(
      (position) => {
        const { latitude, longitude } = position.coords;
        updateWeatherByLocation(latitude, longitude);
      },
      (error) => {
...
updateFutureForecastItems = (weatherData, index) => {
  ...
  const futureWeatherDay = document.querySelectorAll(
    ".future-forecast-item-day"
  );
...
const updateWeatherByLocation = async (lat, lon) => {
  const apiURL = `https://api.openweathermap.org/data/2.5/weather?lat=${lat}&lon=${lon}&appid=${apiKey}&units=imperial`;

  const weatherData = await fetch(apiURL).then((res) => res.json());
  ...
  // Fetch hourly & future forecast using city name
  await updateFutureForecast(cityPlace);
  await updateHourlyForecast(cityPlace);
